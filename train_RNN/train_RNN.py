...
if __name__ == "__main__":
    ...
    model.add(GRU(output_dim=grudim, input_shape=(maxlen,64),activation='tanh',return_sequences=True))
    #model.add(LSTM(output_dim=grudim, input_shape=(maxlen,64),activation='tanh',return_sequences=True))
    model.add(Dropout(0.2))
    model.add(GRU(grudim,activation='tanh',return_sequences=True))
    ...
    model.fit(X,y_train_one_hot,nb_epoch=10, batch_size=512,validation_split=0.1)
    save_model(model)
