    ...
    while time.time()<=run_time:
        ...
        if len(node_index)==0:
            ...
            for i in range(len(node_index)):
                ...
                re.append((-rdock_score[i])/(20+abs(rdock_score[i])))
                """backpropation step"""
            ...
            for i in range(len(node_pool)):
                ...
                while node != None:
                    node.Update(re[i])
                    node = node.parentNode
        with open("outputs/state_backup_{}".format(back_name), "w") as w:
             pickle.dump(state, w)
...
def UCTchemical():
    ...
    time_out=one_search_start_time+60*60*72*100
    #state = chemical()
    #best = MCTS(root = state,verbose = False)
    if os.path.isfile("outputs/state_backup_{}".format(back_name)):
        with open("outputs/state_backup_{}".format(back_name), "r") as r:
            state=pickle.load(r)
    else:
        state = chemical()
    best = MCTS(root = state,verbose = False)
